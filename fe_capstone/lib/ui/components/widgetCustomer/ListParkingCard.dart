...
class ListParkingCard extends StatelessWidget {
  ...
  const ListParkingCard({Key? key, required this.parkingInfor})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return InkWell(
      child: Padding(
        padding:
            EdgeInsets.only(top: 12 * fem, left: 15 * fem, right: 15 * fem),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              parkingInfor.parkingName,
              style: TextStyle(fontSize: 20 * fem, fontWeight: FontWeight.bold),
            ),
            Padding(
              padding: EdgeInsets.only(top: 5 * fem, bottom: 10 * fem),
              child: Text(
                parkingInfor.address,
                style: TextStyle(
                  color: Colors.grey,
                  fontSize: 18 * fem,
                ),
            ...
            Row(
              ...
              children: [
                ...
                Wrap(
                  children: parkingInfor.listMethod.map(
                    (method) {
                      return Padding(
